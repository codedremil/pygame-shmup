# Etape 11 : gestion des vies et des explosions du joueur
import pygame
import random
import os
...
while running:
    ...
    for hit in hits:
        ...
        if player.shield <= 0:
            ...
            # ex11: (à la fin) joue le son de l'explosion
            player_die_sound.play()
            
            # mais si on laisse le "running = False", le jeu se termine trop vite
            #running = False
            # dans un 1er temps: décommenter le kill et commenter la suite
            #player.kill()
            
            # ex1: dans un 2ème temps, on supprime le player.kill()
            # et on masque le joueur
            player.hide()
            player.lives -= 1
            player.shield = 100
